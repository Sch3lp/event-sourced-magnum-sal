...
class MigrationTest {
    ...
    fun `migrate | returns a new EventStream with MinersGotTired events`() {
        ...
        migratedEventStream.map {
            if (it is PlayerActionEvent.SaltMined || it is MinersGotTired) {
                println("${migratedEventStream.indexOf(it)} :        $it")
            }
        }
        assertThat(migratedEventStream)
                .containsOnlyOnce(PlayerActionEvent.SaltMined(White, at(2, -1), Salts(BROWN, GREEN)))
                .containsOnlyOnce(MinersGotTired(White, at(2, -1), 3))
                .containsOnlyOnce(PlayerActionEvent.SaltMined(Black, at(2, -2), Salts(BROWN, BROWN)))
                .containsOnlyOnce(MinersGotTired(Black, at(2, -2), 3))
                .containsOnlyOnce(PlayerActionEvent.SaltMined(Orange, at(2, 1), Salts(BROWN, BROWN)))
                .containsOnlyOnce(MinersGotTired(Orange, at(2, 1), 3))
                .containsOnlyOnce(PlayerActionEvent.SaltMined(Purple, at(2, 2), Salts(BROWN, BROWN)))
...
private fun nextSaltMinedEvent(eventStream: EventStream): PlayerActionEvent.SaltMined {
    ...
    return if (lastMinersGotTired != null) {
        ...
        EventStream(eventsAfterMinersGotTired.toMutableList()).filterEvents<PlayerActionEvent.SaltMined>().first()
    } else {
        eventStream.filterEvents<PlayerActionEvent.SaltMined>().first()
    }
}

private fun tiredWorkersAt(limitedEventStream: EventStream, player: PlayerColor, at: PositionInMine): Int {
    val playersSaltMiningActions = limitedEventStream.filterEvents<PlayerActionEvent.SaltMined>()
